...
// sendWebSocketResponse sends a response back via WebSocket with proper error handling
func sendWebSocketResponse(conn *websocket.Conn, resp ResponseMessage) error {
	...
	log.Printf("Sent response for request %s, success: %t", resp.RequestID, resp.Error == "")
	return nil
}
...
// Helper functions to gather cluster metadata
func getNodeCount() int {
	// TODO: Implement actual node count retrieval
	return 3
}

func getKubernetesVersion() string {
	// TODO: Implement actual Kubernetes version retrieval
	return "v1.28.0"
}
...
func handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		if !ok {
			...
			case "get-namespaces":
				log.Printf("Processing get-namespaces request with data: %+v", req.Request)
				result, execErr = getNamespacesAgent(req.Request)
				if execErr != nil {
					log.Printf("Error getting namespaces: %v", execErr)
				} else {
					log.Printf("Successfully retrieved namespaces, result length: %d chars", len(result))
					// Log a preview of the result for debugging (first 200 chars)
					preview := result
					if len(preview) > 200 {
						preview = preview[:200] + "..."
					}
					log.Printf("Namespace result preview: %s", preview)
				}
				log.Printf("Responding with get-namespaces: completed")
			default:
		...
		if err := sendWebSocketResponse(conn, resp); err != nil {
			log.Printf("Failed to send WebSocket response: %v", err)
			break
		}

		// Send close frame after successful response
