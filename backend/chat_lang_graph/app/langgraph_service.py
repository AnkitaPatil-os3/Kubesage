    ...
    async def get_cluster_info(self, cluster_name: str, user_token: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(verify=False) as client:
                ...
                # FIX: Always initialize data variable first
                data = None
                try:
                    data = response.json()
                ...
                if response.status_code == 200:
                    ...
                    if data and data.get("success") and data.get("cluster"):
                ...
                # Handle all non-200 responses or invalid data
                error_detail = "Unknown error"
                if data:
                    error_detail = data.get("detail", data.get("error", f"HTTP {response.status_code}"))
                else:
                    error_detail = f"HTTP {response.status_code}: {response.text}"
                
                print(f"❌ Failed to get cluster info: {error_detail}")
                logger.error(f"Failed to get cluster info: {error_detail}")
                return None
