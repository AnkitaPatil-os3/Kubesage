...
from app.migrations import ensure_user_exists
...
class ChatService:
    ...
    def create_session(self, user_data: Dict[str, Any], title: str = "Kubernetes Chat") -> ChatSession:
        """Create a new chat session."""
        try:
            # Ensure user exists in database
            user = ensure_user_exists(self.session, user_data)
            ...
            logger.info(f"✅ Created new chat session: {chat_session.session_id} for user {user.id}")
            return chat_session
    ...
    def get_session_history(self, session_id: str) -> List[Dict[str, str]]:
        """Get session history in LangChain format."""
        try:
