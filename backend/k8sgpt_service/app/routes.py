...
async def analyze_cluster(
    ...
    This endpoint triggers a K8sGPT analysis on the currently active Kubernetes cluster
    based on the parameters provided in the request. The analysis will identify potential
    issues and provide explanations and recommendations.
    
    Parameters:
    - **analysis_request**: Contains parameters for the analysis including namespace, filters, etc.
    
    Returns:
    - Analysis result with identified issues, explanations, and recommendations
    
    Raises:
    - 500: If the analysis fails for any reason
    """
...
async def get_analysis(
    ...
    Retrieves the detailed results of a previously run K8sGPT analysis using its unique ID.
    The analysis must belong to the authenticated user.
    
    Parameters:
    - **result_id**: The unique identifier of the analysis result to retrieve
    
    Returns:
    - Detailed analysis result with identified issues, explanations, and recommendations
    
    Raises:
    - 404: If the analysis result is not found
    - 403: If the user doesn't have permission to access this analysis
    """
    user_id = current_user["id"]
...
async def list_analyses(
    ...
    """
    List analysis results for the authenticated user.
    
    Retrieves a paginated list of all K8sGPT analyses performed by the current user,
    with the most recent analyses first.
    ...
    Returns:
    - List of analysis results with their details
...
async def create_backend(
    ...
    Configures a new AI backend for K8sGPT to use for analysis. This includes
    setting up credentials, model parameters, and connection details for the
    specified AI provider.
    
    Parameters:
    - **backend_config**: Configuration details for the AI backend including type,
      API keys, model name, and other provider-specific settings
    
    Returns:
    - The created backend configuration with its ID and status
    
    Raises:
    - 400: If the backend configuration is invalid
    - 500: If the backend configuration fails to be applied
    """
    user_id = current_user["id"]
    ...
    print("base url:", backend_config.baseurl)
    ...
    #     command += f" --apikey {backend_config.api_key}"
    ...
    print("Created backend:", db_backend)
...
async def get_backends(
    ...
    Retrieves all AI backend configurations that have been set up for the
    authenticated user. This includes details about each backend's type,
    model, and default status.
    
    Returns:
    - List of configured AI backends with their details
...
async def get_backend(
    ...
    Retrieves detailed information about a specific AI backend configuration
    by its name. The backend must belong to the authenticated user.
    
    Parameters:
    - **backend_name**: The name of the AI backend to retrieve
    
    Returns:
    - Detailed configuration for the specified AI backend
    
    Raises:
    - 404: If the backend is not found
    - 403: If the user doesn't have permission to access this backend
    """
    user_id = current_user["id"]
...
async def remove_backend(
    ...
    Removes an AI backend configuration from both the database and the K8sGPT
    configuration. This action cannot be undone.
    
    Parameters:
    - **backend_name**: The name of the AI backend to delete
    ...
    Raises:
    - 404: If the backend is not found
    - 500: If the backend deletion fails
    """
    user_id = current_user["id"]
...
async def set_default_backend(
    ...
    Sets the specified AI backend as the default for K8sGPT analysis.
    Only one backend can be the default at a time. This updates both
    the database and the K8sGPT configuration.
    
    Parameters:
    - **backend_name**: The name of the AI backend to set as default
    ...
    Raises:
    - 404: If the backend is not found
    - 500: If setting the default backend fails
    """
    user_id = current_user["id"]
    ...
    # Construct the terminal command
    command = f"k8sgpt auth default -p {backend_name}"
...
async def list_analyzers(
    ...
    Retrieves a list of all available analyzers that K8sGPT can use to
    identify issues in Kubernetes clusters. These analyzers can be specified
    when running an analysis to focus on specific types of problems.
    ...
    Raises:
    - 401: If the user is not authenticated
    - 500: If retrieving the analyzers fails
    """
    user_id = current_user["id"]
