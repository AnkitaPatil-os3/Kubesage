...
async def send_confirmation_email(user_data: Dict, confirm_path: str = "confirm"):
    """
    Send a confirmation email to a new user

    Args:
        user_data: Dictionary containing user information
        confirm_path: The path for confirmation endpoint (default: "confirm")
    ...
    try:
        ...
        yes_url = f"{settings.SERVER_BASE_URL}/api/v1.0/auth/{confirm_path}/{confirmation_id}/yes"
        no_url = f"{settings.SERVER_BASE_URL}/api/v1.0/auth/{confirm_path}/{confirmation_id}/no"
        
        # Check if this is an admin registration
        is_admin = "Super Admin" in user_data.get("roles", "")

        if is_admin:
            # Admin registration email
            subject = "Confirm Your KubeSage Registration"
            email_body = f"""
            <h2>KubeSage Admin Account Registration Confirmation</h2>
            <p>Hello {user_data.get('first_name', '')} {user_data.get('last_name', '')},</p>
            <p>You are being registered as the Super Admin for KubeSage.</p>
            <p>Please confirm to create your admin account:</p>

            <div style="margin-top: 20px;">
                <a href="{yes_url}" style="background-color: #4CAF50; color: white; padding: 10px 20px; text-decoration: none; margin-right: 10px; border-radius: 4px;">Yes, create admin account</a>
                <a href="{no_url}" style="background-color: #f44336; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px;">No, cancel</a>
            </div>
