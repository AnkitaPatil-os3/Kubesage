from fastapi import APIRouter, Depends, HTTPException, File, UploadFile, Query, Form  # Added Form
...
@kubeconfig_router.post("/onboard-cluster", response_model=ClusterConfigResponse, status_code=201,
...
@kubeconfig_router.get("/clusters", response_model=ClusterConfigList,
...
async def list_clusters(
    ...
    return ClusterConfigList(clusters=clusters)
...
async def upload_kubeconfig(
    ...
    provider_name: Optional[str] = Form(None, description="Provider name (e.g., AWS, GCP, Azure, etc.)"),  # New form field
    session: Session = Depends(get_session),
    current_user: Dict = Depends(get_current_user)
):
    """
    ...
    - Accepts provider name from frontend
    ...
    Parameters:
        ...
        provider_name: The cloud provider name (AWS, GCP, Azure, etc.)
    ...
    """
    logger.info(f"Upload request received for user {current_user['id']} with file {file.filename} and provider {provider_name}")
    try:
        ...
        new_kubeconf = Kubeconf(
            ...
            provider_name=provider_name  # Store the provider name from frontend
        )
        ...
        logger.info(f"User {current_user['id']} uploaded kubeconfig: {unique_filename} with provider: {provider_name}")
        ...
        publish_message("kubeconfig_events", {
            ...
            "cluster_name": cluster_name,
            "context_name": context_name,
            "provider_name": provider_name,  # Include provider in event
            "timestamp": datetime.datetime.now().isoformat()
        })
...
async def get_namespaces(
    ...
    server_url = "https://10.0.32.110/k8s/clusters/c-m-872p2drr"
    token = "kubeconfig-user-zfgmlp7tsh:rpz59l6z5pc955xsc68njqf7c7vrrt5dnqpm4whsqnkkxrbfnsndtw"
    # cluster_name = "kubesage-demo"
    
    # logger.info(f"Getting namespaces for user {current_user['id']} from cluster {cluster_name}")
    ...
    try:
        ...
        # Alternative Method 3: If token format is different
        # configuration.api_key_token = token
