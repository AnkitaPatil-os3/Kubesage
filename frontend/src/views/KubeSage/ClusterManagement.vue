...
<template>
  ...
  // API base URL
  const baseUrl = 'https://10.0.32.123:8002/kubeconfig';
  ...
  const toggleActive = async (filename, shouldActivate) => {
    try {
      if (shouldActivate) {
        loading.value = true;
  ...
  const removeConfig = async (filename) => {
    try {
      loading.value = true;
  ...
  const confirmInstallOperator = async () => {
    try {
      loading.value = true;
