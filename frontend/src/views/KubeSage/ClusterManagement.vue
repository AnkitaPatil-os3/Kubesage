...
const uploadFile = async (file) => {
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/kubeconfig/upload`, formData, {
      headers: {
        ...getAuthHeaders(),
...
const toggleActive = debounce(async (filename, shouldActivate) => {
  ...
  try {
    if (shouldActivate) {
      ...
      const response = await axios.put(`${baseUrl}/kubeconfig/activate/${filename}`, {}, {
        headers: getAuthHeaders()
      });
...
const confirmInstallOperator = debounce(async () => {
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/kubeconfig/install-operator`, {}, {
      headers: getAuthHeaders()
    });
