...
const baseurl =  import.meta.env.VITE_CHAT_SERVICE ; 
...
const chatService = {
  ...
  async updateChatSession(sessionId, sessionData) {
    try {
      const response = await axios.patch(`${baseurl}/sessions/${sessionId}`, sessionData, {
        headers: getAuthHeaders(),
      });
      return response.data;
    } catch (error) {
  ...
  async generateSessionTitle(sessionId) {
    try {
      const response = await axios.post(`${baseurl}/sessions/${sessionId}/title`, {}, {
        headers: getAuthHeaders(),
      });
      return response.data;
    } catch (error) {
