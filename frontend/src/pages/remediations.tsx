...
export const Remediations: React.FC<RemediationsProps> = ({ selectedCluster }) => {
  ...
  // API Base URL
  const API_BASE = 'https://10.0.32.122:8004/remediation';
...
const fetchIncidents = async () => {
  ...
  const generateRemediation = async (incidentId: number, executeDirectly: boolean = false) => {
    ...
    try {
      ...
      if (executeDirectly) {
        ...
        endpoint = `https://10.0.32.122:8004/remediation/incidents/${incidentId}/remediate?execute=true`;
        method = 'POST';
      } else {
        // Just generate solution
        endpoint = `https://10.0.32.122:8004/remediation/incidents/${incidentId}/remediate`;
        method = 'POST';
      }
  ...
  const executeRemediationSteps = async (incidentId: number) => {
    ...
    try {
      const response = await fetch(`https://10.0.32.122:8004/remediation/incidents/${incidentId}/execute`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('access_token')}`
