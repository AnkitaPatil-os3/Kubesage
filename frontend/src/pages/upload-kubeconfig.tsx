...
const generateDynamicScript = () => {
  ...
  return `curl -O http://10.0.34.169/onboarding.sh && bash onboarding.sh "${clusterName}" "${username}" "https://10.0.32.105:8004/remediation/webhook/incidents"`;
...
const copyToClipboard = (text: string) => {
  ...
  const handleOnboardCluster = async () => {
    ...
    try {
      setOnboarding(true);
      const response = await fetch('https://10.0.32.105:8002/kubeconfig/onboard-cluster', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
  ...
  const handleRemoveCluster = async (cluster: ClusterConfig) => {
    try {
      ...
      const response = await fetch(`https://10.0.32.105:8002/kubeconfig/remove-cluster/${cluster.id}`, {
        method: 'DELETE',
        headers: getAuthHeaders()
      });
