...
// API Configuration
const API_BASE_URL = 'https://10.0.32.108:8004/api/v1';
...
export const ChatOps: React.FC = () => {
  ...
  const renderMessageContent = (content: string, isStreaming: boolean) => {
    // Check if content contains complete code blocks (matching triple backticks)
    const codeBlockCount = (content.match(/```/g) || []).length;
    const hasCompleteCodeBlock = codeBlockCount % 2 === 0 && codeBlockCount > 0;

    if (isStreaming && !hasCompleteCodeBlock) {
      // Render as plain text with minimal formatting during streaming if code blocks incomplete
      return (
        <pre
          className="whitespace-pre-wrap break-words text-foreground prose-code:text-foreground"
      ...
      // Use our custom RenderMessageContent component for proper code block handling
      return <RenderMessageContent content={content} isStreaming={isStreaming} />;
    }
  };
              ...
              <ScrollShadow orientation="horizontal" className="p-3 sm:p-4">
                <div className="flex gap-3 sm:gap-4">
                  {QUICK_COMMANDS.map((category) => (
                    <Card key={category.category} className="min-w-64 bg-content2 dark:bg-content2 border border-divider flex-shrink-0">
