...
class AdminPasswordReset(BaseModel):
    ...
    @model_validator(mode='before')
    def validate_reset_data(cls, values):
        if not values.get('skip_reset') and not values.get('new_password'):
            raise ValueError('New password is required when not skipping reset')
