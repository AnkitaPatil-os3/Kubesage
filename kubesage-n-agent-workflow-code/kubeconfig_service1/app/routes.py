...
# Core Functions
# -------------------------------------------------
def get_all_node_labels(v1: client.CoreV1Api) -> Dict[str, Dict[str, str]]:
    """Retrieves labels for all nodes in the cluster."""
    nodes = v1.list_node().items
    return {node.metadata.name: node.metadata.labels or {} for node in nodes}

def get_node_labels(v1: client.CoreV1Api, node_name: str) -> Optional[Dict[str, str]]:
...
async def get_all_node_labels_with_insights(v1: client.CoreV1Api) -> List[Dict]:
    """Retrieves labels and generates AI insights for all nodes."""
    all_labels = get_all_node_labels(v1)
    nodes_data = []
...
async def get_node_labels_with_insights(v1: client.CoreV1Api, node_name: str) -> Optional[Dict]:
    """Retrieves labels and generates AI insights for a specific node."""
    labels = get_node_labels(v1, node_name)
    if labels is None:
        return None
...
def get_all_node_details(v1: client.CoreV1Api) -> List[Dict]:
...
def get_node_details(v1: client.CoreV1Api, node_name: str) -> Optional[Dict]:
...
@cluster_router.get("/clusters/{cluster_id}/labels", response_model=AllNodeLabels,
                   summary="Get All Node Labels",
                   description="Retrieves labels for all nodes in the specified cluster")
async def get_all_labels(
    ...
    """
    Retrieves labels for all nodes in the specified cluster.
    """
    logger.info(f"User {current_user['id']} requested all node labels from cluster ID {cluster_id}")
    ...
    try:
        ...
        # Get all node labels
        all_labels = get_all_node_labels(v1)
        nodes = [{"node_name": name, "labels": labels} for name, labels in all_labels.items()]

        logger.info(f"Successfully retrieved labels for {len(nodes)} nodes from cluster '{cluster.cluster_name}'")

        return AllNodeLabels(nodes=nodes)

    except client.rest.ApiException as e:
        logger.error(f"Kubernetes API error for cluster ID {cluster_id}: Status={e.status}, Reason={e.reason}")
        raise HTTPException(status_code=500, detail=f"Kubernetes API error: {e.reason}")
    except Exception as e:
        logger.error(f"Error retrieving node labels from cluster ID {cluster_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error retrieving node labels: {str(e)}")

@cluster_router.get("/clusters/{cluster_id}/labels/{node_name}", response_model=NodeLabels,
                   summary="Get Node Labels by Name",
                   description="Retrieves labels for a specific node in the specified cluster")
async def get_node_labels_endpoint(
    ...
    """
    Retrieves labels for a specific node in the specified cluster.
    """
    logger.info(f"User {current_user['id']} requested labels for node '{node_name}' from cluster ID {cluster_id}")
    ...
    try:
        ...
        logger.info(f"Successfully retrieved labels for node '{node_name}' from cluster '{cluster.cluster_name}'")

        return NodeLabels(node_name=node_name, labels=labels)
...
@cluster_router.get("/clusters/{cluster_id}/labels/insights", response_model=AllNodeLabels,
                   summary="Get All Node Labels with AI Insights",
                   description="Retrieves labels and AI-generated insights for all nodes in the specified cluster")
async def get_all_labels_with_insights(
    cluster_id: int,
    session: Session = Depends(get_session),
    current_user: Dict = Depends(require_permission("clusters"))
):
    """
    Retrieves labels and AI-generated insights for all nodes in the specified cluster.
    """
    logger.info(f"User {current_user['id']} requested all node labels with insights from cluster ID {cluster_id}")
    ...
    try:
        ...
        # Get all node labels with insights
        nodes_data = await get_all_node_labels_with_insights(v1)
        nodes = [{"node_name": node["node_name"], "labels": node["labels"], "insights": node["insights"]} for node in nodes_data]

        logger.info(f"Successfully retrieved labels with insights for {len(nodes)} nodes from cluster '{cluster.cluster_name}'")
        ...
        logger.error(f"Error retrieving node labels with insights from cluster ID {cluster_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error retrieving node labels with insights: {str(e)}")

@cluster_router.get("/clusters/{cluster_id}/details", response_model=AllNodeDetails,
                   summary="Get All Node Details",
                   description="Retrieves key details for all nodes in the specified cluster")
async def get_all_node_details_endpoint(
    cluster_id: int,
    session: Session = Depends(get_session),
    current_user: Dict = Depends(require_permission("clusters"))
):
    """
    Retrieves key details for all nodes in the specified cluster.
    """
    logger.info(f"User {current_user['id']} requested all node details from cluster ID {cluster_id}")
    ...
    try:
        ...
        nodes_data = get_all_node_details(v1)
        nodes = [{"node_name": node["node_name"], "details": node["details"]} for node in nodes_data]

        logger.info(f"Successfully retrieved details for {len(nodes)} nodes from cluster '{cluster.cluster_name}'")

        return AllNodeDetails(nodes=nodes)

    except client.rest.ApiException as e:
        logger.error(f"Kubernetes API error for cluster ID {cluster_id}: Status={e.status}, Reason={e.reason}")
        raise HTTPException(status_code=500, detail=f"Kubernetes API error: {e.reason}")
    except Exception as e:
        logger.error(f"Error retrieving node details from cluster ID {cluster_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error retrieving node details: {str(e)}")

@cluster_router.get("/clusters/{cluster_id}/details/{node_name}", response_model=NodeDetails,
                   summary="Get Node Details by Name",
                   description="Retrieves key details for a specific node in the specified cluster")
async def get_node_details_endpoint(
    ...
    """
    Retrieves key details for a specific node in the specified cluster.
    """
    logger.info(f"User {current_user['id']} requested details for node '{node_name}' from cluster ID {cluster_id}")
    ...
    try:
        ...
        # Get node details
        node_data = get_node_details(v1, node_name)
        if node_data is None:
            raise HTTPException(status_code=404, detail=f"Node '{node_name}' not found")

        logger.info(f"Successfully retrieved details for node '{node_name}' from cluster '{cluster.cluster_name}'")

        return NodeDetails(node_name=node_name, details=node_data["details"])
